...
public class MockProcessor extends AuditablePaymentProcessor {
    ...
    public PaymentResponse process(PaymentRequest request) {
        if (request == null) throw new ApiException("MockProcessor.Request", "La Request no puede ser nula");
        if (request.getAmount() < 0)
            throw new ApiException("MockProcessor.Request.amount", "El monto no puede ser negativo");
